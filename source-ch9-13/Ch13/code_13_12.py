...
img_edge1 = cv2.adaptiveThreshold(img_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                cv2.THRESH_BINARY,   blockSize=9, C=5)
img_edge2 = cv2.adaptiveThreshold(img_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                cv2.THRESH_BINARY,   blockSize=9, C=5)

# 결과는 윤곽선 픽셀이 드러난다
cv2.imshow('edge', img_edge1)
cv2.imshow('edge - gaussian', img_edge2)
